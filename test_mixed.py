"""Test mixed JFla+COOMO fine-tuned GPT-SoVITS model"""
import os
import sys
...
# Use fine-tuned GPT + fine-tuned SoVITS (mixed)
GPT_MODEL = os.path.join(GPT_SOVITS_DIR, "GPT_weights_v2", "sion_mixed-e15.ckpt")
SOVITS_MODEL = os.path.join(GPT_SOVITS_DIR, "SoVITS_weights_v2", "sion_mixed_e20.pth")

# Pick a good reference audio (3-5s sliced segment)
...
print(f"\nLoading fine-tuned GPT + SoVITS (mixed)...")
change_gpt_weights(gpt_path=GPT_MODEL)
change_sovits_weights(sovits_path=SOVITS_MODEL)
print("Models loaded!")

output_dir = os.path.join(GPT_SOVITS_DIR, "test_output_mixed")
os.makedirs(output_dir, exist_ok=True)

test_cases = [
    ("안녕하세요, 저는 시온이에요. 반가워요!", "한국어", "sion_mixed_kr_01"),
    ("오늘 방송 재미있게 할게요, 많이 응원해주세요!", "한국어", "sion_mixed_kr_02"),
    ("와 진짜요? 대박이다! 너무 좋아요!", "한국어", "sion_mixed_kr_03"),
    ("Hello everyone! I'm Sion, it's so nice to meet you all today!", "영어", "sion_mixed_en_01"),
]
...
# Also test with pretrained GPT + mixed SoVITS for comparison
print(f"\n{'='*60}")
print("Testing with PRETRAINED GPT + mixed SoVITS...")
...
for text, lang, filename_base in [
    ("안녕하세요, 저는 시온이에요. 반가워요!", "한국어", "sion_mixed_kr_pretrained_01"),
    ("Hello everyone! I'm Sion, it's so nice to meet you!", "영어", "sion_mixed_en_pretrained_01"),
]:
    print(f"\nGenerating [{lang}]: {text}")
    try:
        result = get_tts_wav(
