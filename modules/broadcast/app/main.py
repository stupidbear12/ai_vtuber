...
@app.get("/chzzk/live-status")
async def chzzk_live_status():
    """현재 방송 중인지 여부를 반환한다 (GET /open/v1/lives 위임)."""
    try:
        status = await _chzzk_client.get_live_status()
    except RuntimeError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def chzzk_live_status_raw():
    ...
    headers = _chzzk_client._client_headers()
    async with aiohttp.ClientSession() as s:
        async with s.get(
            f"{CHZZK_BASE_URL}/open/v1/lives",
            headers=headers,
            timeout=aiohttp.ClientTimeout(total=10.0),
        ) as r:
            text = await r.text()
            return {"http_status": r.status, "raw_response": json.loads(text) if r.status == 200 else text}
