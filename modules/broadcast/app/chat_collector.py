...
리팩토링 구조:
  - tts_manager.py      : TTSMixin (TTS 생성 + Live2D 브로드캐스트)
  - music_handler.py    : MusicHandlerMixin (음악 명령 처리 + Auto-DJ)
  - autonomous_planner.py: AutonomousPlannerMixin (자율 행동 플래너)
"""
...
from app.tts_manager import TTSMixin
from app.music_handler import MusicHandlerMixin, AUTO_DJ_ENABLED
from app.autonomous_planner import (
    AutonomousPlannerMixin,
    AUTONOMOUS_ENABLED,
    AUTONOMOUS_HISTORY_SIZE,
    AUTONOMOUS_CRAWL_SUMMARY_SIZE,
)
...
class BroadcastChatManager(TTSMixin, MusicHandlerMixin, AutonomousPlannerMixin):
    ...
    믹스인 구조:
      - TTSMixin: _speak_and_broadcast
      - MusicHandlerMixin: _is_music_command, _handle_music_command,
                           _announce_music, _handle_music_action,
                           _auto_dj_loop, _auto_dj_safe
      - AutonomousPlannerMixin: _enforce_action_rules, _ask_llm_action,
                                _execute_action, _action_search, _action_talk,
                                _action_music, _action_react, _action_topic_change,
                                _autonomous_worker, _radio_worker_safe,
                                _store_crawl_to_rag
    """
    ...
    def __init__(
        ...
        # TTS 직렬화 락 -- 동시에 하나의 TTS만 생성+재생되도록 보장
        ...
        self._surf_active: bool = False          # 웹서핑 처리 중 여부 (라디오 일시 중지용)

        # 자율 행동 플래너 상태
        self._autonomous_active: bool = False    # 자율 행동 실행 중 여부
        self._action_history: Deque[dict] = deque(maxlen=AUTONOMOUS_HISTORY_SIZE)
        self._crawl_results: Deque[str] = deque(maxlen=AUTONOMOUS_CRAWL_SUMMARY_SIZE)
        self._current_topic: str = ""

        # 통계: 수신/응답/건너뜀/오류/라디오/자율행동 횟수
        self.stats: dict = {
            "received": 0,
            "responded": 0,
            "skipped": 0,
    ...
    async def _handle_surf_command(self, msg: ChatMessage) -> None:
        ...
        웹서핑 중에는 _surf_active 플래그를 설정하여
        라디오 자동 멘트가 TTS를 덮어쓰지 않도록 한다.
        """
        surf_url = os.environ.get("AI_BROWSER_AGENT_URL", "http://localhost:8007")
        self._surf_active = True
        logger.info("[ChatManager] 웹서핑 시작 → 라디오 자동 멘트 일시 중지")
        try:
            async with aiohttp.ClientSession() as session:
            ...
            logger.info("[ChatManager] 웹서핑 종료 → 라디오 자동 멘트 재개")
    ...
    async def _respond_to_chat(self, msg: ChatMessage) -> None:
        ...
        # 웹 크롤링 결과가 있으면 컨텍스트에 추가 (브라우저 내용 참조)
        if self._crawl_results:
            crawl_lines = [f"  - {r}" for r in self._crawl_results]
            crawl_section = "\n[현재 브라우저에서 본 내용]\n" + "\n".join(crawl_lines)
            chat_context = (chat_context + "\n" + crawl_section) if chat_context else crawl_section
        ...
        # ── Step 3+4: TTS 생성 + 브로드캐스트 (락으로 직렬화) ──────
    ...
    async def start(self, platform: str, channel_id: str) -> None:
        ...
        self.stats = {"received": 0, "responded": 0, "skipped": 0, "errors": 0, "radio_talks": 0, "autonomous_actions": 0}
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "surf_active": self._surf_active,
            "autonomous_active": self._autonomous_active,
            "autonomous_enabled": AUTONOMOUS_ENABLED,
            "current_topic": self._current_topic,
            "action_history": list(self._action_history),
            "crawl_results_count": len(self._crawl_results),
