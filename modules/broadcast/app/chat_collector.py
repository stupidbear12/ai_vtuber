...
AUTONOMOUS_HISTORY_SIZE = 10       # 최근 행동 히스토리 보관 개수
AUTONOMOUS_CRAWL_SUMMARY_SIZE = 5  # 크롤링 결과 요약 보관 개수
...
class BroadcastChatManager:
    ...
    async def _ask_llm_action(self) -> dict:
        ...
        prompt = (
            ...
            f"   카테고리 예시: 과학기술, 맛집/음식, 여행, 게임, 스포츠, 음악, 우주, 역사, 동물, 패션, IT뉴스, 밈/유머, 심리학, 건강\n"
    ...
    async def _action_react(self, content: str) -> None:
        ...
        react_message = (
            f"[자율 방송 모드] 이전에 검색한 내용에 대해 감상을 말해주세요.\n"
            f"검색 결과:\n{crawl_context}\n"
            f"시온의 시각: {content}"
        )
    ...
    async def _autonomous_worker(self) -> None:
        ...
        기존 _radio_worker를 대체하는 에이전트 루프.
